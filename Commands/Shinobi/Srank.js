...
module.exports.run = async (bot, message, args) => 
{
    ...
    let enemyData = bot.set.shinobi.srank//All enemies
    ...
    if (defeatedenemies.includes(enemyData[10].name))
        text11 = `${bot.set.check}`
    else
        text11 = `❌`

    if (defeatedenemies.includes(enemyData[11].name))
        text12 = `${bot.set.check}`
    else
        text12 = `❌`
...
**11. ${enemyData[10].name}** ${text11}
**12. ${enemyData[11].name}** ${text12}
        `)
    ...
    const background = await Canvas.loadImage('https://i.ibb.co/xhsg1d1/Battle-Template-S-Rank.png')
    ...
    const attachment = new Discord.MessageAttachment(canvas.toBuffer(), "srank.jpg")

    const embed = new Discord.MessageEmbed()
        .setColor('00e3ff')
        .setAuthor(message.author.tag, message.author.avatarURL({dynamic: true}))
        .attachFiles(attachment)
        .setImage('attachment://srank.jpg')
    ...
    while (myHealth > 0 && enemyHealth > 0){
        ...
        if (!reply.first()){//What happens if user does not enter anything
            ...
            if (enemyHealth < 1){//Wining battle condition
                ...
                embed.setColor('00e3ff')
                bot.rpg.add(`${message.author.id}.srank`, 1)
