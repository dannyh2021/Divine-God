...
module.exports.run = async (bot, message, args) => 
{
    ...
    let enemyData = bot.set.shinobi.crank//All enemies
...
**10. ${enemyData[9].name}** ${text10}
    ...
    const background = await Canvas.loadImage('https://i.ibb.co/XZrZy2X/Battle-Template-C-Rank.png')
    ...
    const attachment = new Discord.MessageAttachment(canvas.toBuffer(), "crank.jpg")

    const embed = new Discord.MessageEmbed()
        .setColor('00e3ff')
        .setAuthor(message.author.tag, message.author.avatarURL({dynamic: true}))
        .attachFiles(attachment)
        .setImage('attachment://crank.jpg')
    ...
    while (myHealth > 0 && enemyHealth > 0){
        ...
        if (!reply.first()){//What happens if user does not enter anything
            ...
            if (enemyHealth < 1){//Wining battle condition
                ...
                embed.setColor('00e3ff')
                bot.rpg.add(`${message.author.id}.crank`, 1)
