...
module.exports.run = async (bot, message, args) => 
{
    ...
    let enemyData = bot.set.shinobi.brank//All enemies
...
**10. ${enemyData[9].name}** ${text10}
    ...
    const background = await Canvas.loadImage('https://i.ibb.co/1R84dqw/Battle-Template-B-Rank.png')
    ...
    const attachment = new Discord.MessageAttachment(canvas.toBuffer(), "brank.jpg")

    const embed = new Discord.MessageEmbed()
        .setColor('00e3ff')
        .setAuthor(message.author.tag, message.author.avatarURL({dynamic: true}))
        .attachFiles(attachment)
        .setImage('attachment://brank.jpg')
    ...
    while (myHealth > 0 && enemyHealth > 0){
        ...
        if (!reply.first()){//What happens if user does not enter anything
            ...
            if (enemyHealth < 1){//Wining battle condition
                ...
                embed.setColor('00e3ff')
                bot.rpg.add(`${message.author.id}.brank`, 1)
