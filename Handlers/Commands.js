const { getFilesRecursive } = require('../Util/utils.js');
...
module.exports = function(bot) {
    ...
    commands.forEach(file => {      
        ...
        console.log(`${cmd.config.command} ${space}`);//file.substring(file.lastIndexOf('/') + 1, file.length) == cmd name from path
...
module.exports.reloadCategory = function(bot, category) {
    const commands = getFilesRecursive(`${process.cwd()}/Commands/${category}`);
